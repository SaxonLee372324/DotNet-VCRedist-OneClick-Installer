...
# -----------------------------
# Enable / check .NET Framework
# -----------------------------
function Test-NetFramework48OrNewer {
...
function Enable-NetFrameworkFeature {
    ...
    try {
        ...
        if ($Name -eq "4.8") {
            if (Test-NetFramework48OrNewer) {
                Write-Host (CN 0x2E,0x4E,0x45,0x54,0x20,0x34,0x2E,0x38,0x20,0x6216,0x66F4,0x9AD8,0x7248,0x672C,0x5DF2,0x5B89,0x88C5)
            } else {
                Write-Host (CN 0x2E,0x4E,0x45,0x54,0x20,0x34,0x2E,0x38,0x20,0x672A,0x68C0,0x6D4B,0x5230,0xFF0C,0x6B63,0x5728,0x542F,0x7528)

                $feature = Get-WindowsOptionalFeature -Online -FeatureName NetFx4-AdvSrvs -ErrorAction SilentlyContinue
                ...
                if (Test-NetFramework48OrNewer) {
                    ...
                    Write-Host (CN 0x5C06,0x4F7F,0x7528,0x20,0x77,0x69,0x6E,0x67,0x65,0x74,0x20,0x5B89,0x88C5,0x20,0x2E,0x4E,0x45,0x54,0x20,0x46,0x72,0x61,0x6D,0x65,0x77,0x6F,0x72,0x6B,0x20,0x34,0x2E,0x38,0x2B)
                    winget install `
                        --id Microsoft.DotNet.Framework.Runtime `
                        --source winget `
                        --silent `
                    ...
                    if (Test-NetFramework48OrNewer) {
                        ...
                        Write-Host (CN 0x2E,0x4E,0x45,0x54,0x20,0x34,0x2E,0x38,0x20,0x5B89,0x88C5,0x540E,0x4ECD,0x672A,0x68C0,0x6D4B,0x5230) -ForegroundColor Yellow
                    }
                } else {
                    Write-Host (CN 0x672A,0x68C0,0x6D4B,0x5230,0x20,0x77,0x69,0x6E,0x67,0x65,0x74,0xFF0C,0x8DF3,0x8FC7,0x20,0x2E,0x4E,0x45,0x54,0x20,0x34,0x2E,0x38,0x2B,0x20,0x5B89,0x88C5) -ForegroundColor Yellow
                }
            }
        }
    }
